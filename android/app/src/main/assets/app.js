...
let thinking = false, engineReady = false, worker = null, startedAt = Date.now(), toastTimer;
...
function startWorker(){
  ...
  worker.onerror = event => engineError(event.message || "Web Worker 启动失败");
  worker.postMessage({type:"init"});
}

function command(text){ worker.postMessage({type:"command", data:text}); }
...
function onEngineMessage(event){
  ...
  if (type === "ready") {
    ...
    engineError(type === "exit" ? `引擎已退出 (${data})` : data);
  }
}

function engineError(message){
  thinking = false; engineReady = false;
  ui.engineBadge.className = "badge error";
  ui.engineBadge.innerHTML = "<span></span>引擎异常";
  showToast(message || "Rapfi 引擎异常"); updateStatus();
}
...
function updateStatus(){
  ...
  else if(thinking){ui.statusTitle.textContent="Rapfi 思考中";ui.statusKicker.textContent="正在计算最佳落点…";}
